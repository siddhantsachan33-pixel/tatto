...
async function main() {
  try {
    // 1. Get original products.js content from Git HEAD
    console.log('Retrieving original products.js from git history...');
    let originalContent = '';
    try {
      originalContent = execSync('git show HEAD:src/data/products.js', { encoding: 'utf8' });
    } catch (e) {
      console.log('Git show failed, attempting to read git diff or local backups...');
      // Fallback: If git fails, let's look for standard mock data or try to read the file if we can
    }

    if (!originalContent) {
      console.error('Could not retrieve original products.js from git history.');
      return;
    }

    // Extract categories, faqs, and bundles from the original content
    // We search for: export const categories = [ ... ]; export const faqs = [ ... ]; export const bundles = [ ... ];
    const extractArray = (varName) => {
      const startIndex = originalContent.indexOf(`export const ${varName} = [`);
      ...
      for (let i = startIndex; i < originalContent.length; i++) {
        ...
        if ((char === '"' || char === "'") && originalContent[i-1] !== '\\') {
          if (!inString) {
            inString = true;
            stringChar = char;
        ...
        if (!inString) {
          ...
          if (char === ']') {
            bracketCount--;
            if (bracketCount === 0) {
              return originalContent.substring(startIndex, i + 2); // include the semicolon/newline
            }
          }
        }
      }
  ...
  { id: 'abstract', name: 'Abstract', icon: '🎨' }
    ...
    const faqsBlock = extractArray('faqs') || `export const faqs = [
  {
  ...
  },
  {
    ...
    const bundlesBlock = extractArray('bundles') || `export const bundles = [
  {
    id: "b1",
    name: "Classic Minimalist Duo",
    description: "Get our two most popular fine-line minimalist tattoos for a clean look.",
    price: 499,
    originalPrice: 799
  }
];`;
    ...
    if (!res.ok) {
      throw new Error(`Failed to fetch inkup products: ${res.status}`);
    }
